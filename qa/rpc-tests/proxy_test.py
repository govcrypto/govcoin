...
class ProxyTest(BitcoinTestFramework):
    ...
    def node_test(self, node, proxies, auth):
        ...
        node.addnode("[1233:7472:2434:2747:3234:2345:6546:4534]:5443", "onetry")
        cmd = proxies[1].queue.get()
        assert(isinstance(cmd, Socks5Command))
        # Note: bitcoind's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
        assert_equal(cmd.atyp, AddressType.DOMAINNAME)
        assert_equal(cmd.addr, "1233:7472:2434:2747:3234:2345:6546:4534")
        ...
        node.addnode("youraddress.onion:7474", "onetry")
        cmd = proxies[2].queue.get()
        assert(isinstance(cmd, Socks5Command))
        assert_equal(cmd.atyp, AddressType.DOMAINNAME)
        assert_equal(cmd.addr, "youraddress.onion")
        assert_equal(cmd.port, 7474)
        if not auth:
            assert_equal(cmd.username, None)
            assert_equal(cmd.password, None)
        rv.append(cmd)
